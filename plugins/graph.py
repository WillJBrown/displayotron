...
class GraphNetTrans(MenuOption):
    ...
    def get_down(self):
        show_dl_raw = ""
        show_dl_hr = "ifconfig eth0 | grep bytes | grep RX | cut -d' ' -f14"
        hr_dl = run_cmd(show_dl_hr).decode()
        return hr_dl

    def get_up(self):
        show_ul_raw = ""
        show_ul_hr = "ifconfig eth0 | grep bytes | grep TX | cut -d' ' -f14"
        hr_ul = run_cmd(show_ul_hr).decode()
        return hr_ul
    ...
    def redraw(self, menu):
        ...
        menu.write_row(1,'Dn:' + self.get_down()[:-1] + ' bytes')
        menu.write_row(2,'Up:' + self.get_up()[:-1] + ' bytes')
