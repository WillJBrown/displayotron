...
class Menu():
  ...
  * A class derived from MenuOption, providing interactive functionality
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.config_file = os.path.join(utils.get_script_dir(),'dothat.cfg')
  ...
  def save(self):
    if sys.version_info[0] >= 3:
      ...
      with open(self.config_file, 'wb') as configfile:
        self.config.write(configfile)
#    print('Config saved to {}'.format(self.config_file))
    backlight.off()
    lcd.set_contrast(0)
    lcd.clear()
  ...
  def redraw(self): 
    ...
    if self.mode == _MODE_NAV:
      ...
      if len(self.current_submenu()) > 2:
        self.write_option(
          row=2,
          text=self.get_menu_item(self.two_position_on()),
          margin=1
        )
      else:
        self.clear_row(2)
